...
Future<ResponseModel> networkRequest(
  ...
  try {
    ...
    final res =
        response.body.isNotEmpty ? jsonDecode(const Utf8Decoder().convert(response.bodyBytes)) as Map<String, dynamic> : {'message': 'Success'};
    ...
    return ResponseModel(message: e.message.toString(), status: false, statusCode: 408);
  } on SocketException catch (e) {
    return ResponseModel(message: e.message, status: false, statusCode: 408);
  } on Exception {
    return const ResponseModel(message: 'Connection Problem! 😐', status: false, statusCode: 500);
  }
}
